...
impl Decimal {
    ...
    pub fn to_scaled_val(&self) -> Result<u128, ProgramError> {
        Ok(u128::try_from(self.0).map_err(|_| TroveError::MathOverflow)?)
    }
    ...
    pub fn try_round_u64(&self) -> Result<u64, ProgramError> {
        let rounded_val = Self::half_wad()
            .checked_add(self.0)
            .ok_or(TroveError::MathOverflow)?
            .checked_div(Self::wad())
            .ok_or(TroveError::MathOverflow)?;
        Ok(u64::try_from(rounded_val).map_err(|_| TroveError::MathOverflow)?)
    }
    ...
    pub fn try_ceil_u64(&self) -> Result<u64, ProgramError> {
        let ceil_val = Self::wad()
            ...
            .checked_div(Self::wad())
            .ok_or(TroveError::MathOverflow)?;
        Ok(u64::try_from(ceil_val).map_err(|_| TroveError::MathOverflow)?)
    }
...
impl TryAdd for Decimal {
    fn try_add(self, rhs: Self) -> Result<Self, ProgramError> {
        Ok(Self(
            self.0.checked_add(rhs.0).ok_or(TroveError::MathOverflow)?,
        ))
    }
}
...
impl TrySub for Decimal {
    fn try_sub(self, rhs: Self) -> Result<Self, ProgramError> {
        Ok(Self(
            self.0.checked_sub(rhs.0).ok_or(TroveError::MathOverflow)?,
        ))
    }
}
